...
export default function OrdersPage() {
  ...
  const calculateCodTotal = useCallback(
    (order: Order) =>
      getDisplayCodTotal({ items: order.orderItems, depositAmount: order.depositAmount }),
    [],
  );
  const calculateProfit = useCallback(
    (order: Order) =>
      order.orderItems.reduce(
        (acc, item) => acc + (item.salePrice - item.basePrice) * item.quantity,
        0,
      ),
    [],
  );
  const calculateCost = useCallback(
    (order: Order) => order.orderItems.reduce((acc, item) => acc + item.basePrice * item.quantity, 0),
    [],
  );
  ...
  const submitStatusUpdate = async () => {
    ...
    if (newStatus === 'SHIPPING' && !trackingCode) {
      return toast.error('Vui lòng nhập mã vận đơn khi trạng thái là Đang vận chuyển');
    }
    if (newStatus === 'DONE' && !confirmPayment) {
      return toast.error('Vui lòng xác nhận khách đã thanh toán đầy đủ');
    }
      ...
      {order.customerPhone ? (
        <div className="mt-1 flex items-center gap-1.5 text-sm text-muted-foreground">
          <button
            ...
            className="rounded-xl border border-border/60 bg-white p-1.5 text-slate-400 transition-colors hover:text-primary"
      ...
      {order.customerAddress ? (
        <div className="mt-1.5 flex items-start gap-1.5 text-xs text-slate-600">
          <button
            ...
            className="shrink-0 rounded-xl border border-border/60 bg-white p-1.5 text-slate-400 transition-colors hover:text-primary"
  ...
  return (
    <div className="space-y-6 md:space-y-8">
      <section className="page-hero px-5 py-6 sm:px-6 md:px-8">
        <div className="relative z-10 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl">
            <p className="section-kicker">Điều phối đơn hàng</p>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            <div className="flex items-center rounded-full border border-primary/15 bg-white/80 px-4 py-2 text-sm text-muted-foreground">
            ...
            {filteredOrders.length === 0 ? (
              ...
              filteredOrders.map((order) => (
                <TableRow key={order.id} className="border-border/60">
                  ...
                  <TableCell className="whitespace-nowrap">
                    <div className="flex items-center gap-2 font-medium text-primary">
                      ...
                      <button
                        ...
                        className="rounded-xl border border-border/60 bg-white p-1.5 text-slate-400 transition-colors hover:text-foreground"
                      >
                        <Chevron expanded={expandedOrders.has(order.id)} />
                      </button>
                    </div>
                  ...
                  <TableCell>
                    <span
                      className={`inline-flex items-center rounded-full border px-3 py-1 text-xs font-semibold ${getStatusStyles(order.status)}`}
                    >
                      {getStatusText(order.status)}
                    </span>
                    {order.trackingCode ? (
                      <div className="mt-1 text-xs text-muted-foreground">Mã: {order.trackingCode}</div>
                    ) : null}
                  </TableCell>
                  <TableCell>
                    {order.hasDeposit ? (
                  ...
                  <TableCell>
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <button
                        ...
                        className="rounded-xl border border-border/70 p-2 text-slate-400 transition-colors hover:border-red-100 hover:bg-red-50 hover:text-red-500"
                        title="Xóa đơn hàng"
                      >
      ...
      <div className="space-y-4 md:hidden">
        {filteredOrders.length === 0 ? (
          <div className="surface-panel p-8 text-center text-gray-500">Không tìm thấy đơn hàng nào</div>
        ) : (
          filteredOrders.map((order) => (
            <article key={order.id} className="surface-panel space-y-4 p-4">
              <div className="flex items-start justify-between gap-4 border-b border-border/60 pb-4">
                ...
                <div className="min-w-[7.5rem] text-right">
                  ...
                  <div className="mt-2">
                    <span
                      className={`inline-flex items-center rounded-full border px-3 py-1 text-[10px] font-semibold ${getStatusStyles(order.status)}`}
                    >
                      {getStatusText(order.status)}
                    </span>
                  </div>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <div className="rounded-[1.35rem] border border-border/70 bg-secondary/55 px-4 py-3">
                  <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground">Đặt cọc</p>
                  <p className="mt-2 text-lg font-semibold tracking-[-0.03em] text-foreground">
                    {formatCurrency(order.depositAmount || 0)}
                  </p>
                  <p className="mt-1 text-xs text-muted-foreground">
                    {order.hasDeposit ? 'Đơn đã có cọc' : 'Đơn chưa ghi nhận cọc'}
                  </p>
                </div>
              </div>

              {order.trackingCode ? (
                <div className="rounded-[1.25rem] border border-border/70 bg-white px-3 py-3 text-xs font-medium text-primary">
                  Vận đơn: {order.trackingCode}
                </div>
              ) : null}

              <div className="grid grid-cols-[auto_1fr_1fr] gap-2 border-t border-border/60 pt-3">
                <button
                  onClick={() => handleDeleteOrder(order.id)}
                  className="rounded-2xl border border-border/70 p-2.5 text-slate-400 transition-colors hover:border-red-100 hover:bg-red-50 hover:text-red-500"
                  title="Xóa đơn"
                >
                  <Trash2 className="h-4 w-4" />
                </button>
              ...
              <Button variant="outline" size="sm" className="w-full" onClick={() => handleOpenUpdate(order)}>
                Cập nhật trạng thái
              </Button>
                ...
                <div className="space-y-3 border-t border-border/60 pt-4 text-sm">
                  <div className="space-y-2 rounded-[1.35rem] border border-border/70 bg-[#fffaf3] p-3">
                    {order.orderItems.map((item) => (
                      <div
                        key={item.id}
                        className="flex justify-between border-b border-border/50 pb-2 text-gray-700 last:border-0 last:pb-0"
                      >
                        <span>
                          {item.quantity}x {item.product.name}
                        </span>
                  ...
                  <div className="flex justify-between px-1 text-sky-700">
                    <span>Tiền lấy hàng:</span>
                    <span>{formatCurrency(calculateCost(order))}</span>
                  </div>
                  <div className="flex justify-between px-1 font-bold text-primary">
                    <span>Tiền lời:</span>
        ...
        <DialogContent className="z-[100] flex h-screen w-screen max-w-none items-center justify-center border-none bg-black/90 p-0">
          {receiptUrl ? (
            <div className="relative flex h-full w-full items-center justify-center p-4">
              <Image src={receiptUrl} alt="Biên lai thanh toán" fill className="object-contain" priority />
            </div>
          ) : null}
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isUpdateOpen} onOpenChange={setIsUpdateOpen}>
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            ...
            {newStatus === 'DONE' ? (
              <div className="rounded-[1.2rem] border border-rose-100 bg-rose-50 p-3 text-sm text-rose-900">
                Nếu chuyển trạng thái sang &quot;Đã xong&quot;, ảnh hóa đơn cọc sẽ bị xóa khỏi hệ thống để tiết kiệm dung lượng.
              </div>
            ) : null}
            ...
            {newStatus === 'DONE' ? (
              <div className="flex items-start space-x-3 rounded-[1.2rem] border border-emerald-100 bg-emerald-50 p-3">
                <input
                  type="checkbox"
                  id="confirm-payment"
                ...
                />
                <label htmlFor="confirm-payment" className="cursor-pointer text-sm font-medium text-emerald-900">
      ...
      <Dialog open={isBillOpen} onOpenChange={setIsBillOpen}>
        <DialogContent className="invoice-print-shell max-h-[90vh] max-w-2xl overflow-y-auto">
          ...
          {billOrder ? (
            <div
              id="bill-content"
              className="invoice-print-card space-y-6 rounded-[1.75rem] border border-border/70 bg-white p-6 text-slate-800"
            >
              <div className="border-b border-border/70 pb-4 text-center">
                ...
                <p className="mt-1 text-sm text-slate-500">
                  {new Date(billOrder.createdAt).toLocaleDateString('vi-VN')}
                </p>
              </div>

              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Khách hàng</p>
                  <p className="text-base font-semibold">{billOrder.customerName}</p>
                  {billOrder.customerPhone ? <p>{billOrder.customerPhone}</p> : null}
                  {billOrder.customerAddress ? (
                    <p className="mt-1 text-xs italic text-slate-500">{billOrder.customerAddress}</p>
                  ) : null}
                </div>
                <div className="text-right">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Cửa hàng</p>
                ...
                <table className="w-full text-sm">
                  <thead className="border-b bg-slate-50">
                    <tr className="text-[11px] uppercase tracking-tight">
                      ...
                      <th className="w-10 px-2 py-1 text-center">SL</th>
                  ...
                  <tbody className="text-[12px]">
                    {billOrder.orderItems.map((item) => (
                      <tr key={item.id} className="border-b last:border-0">
                        <td className="px-2 py-1 font-medium leading-tight">{item.product.name}</td>
                        <td className="px-2 py-1 text-center">{item.quantity}</td>
                        <td className="px-2 py-1 text-right">{formatCurrency(item.salePrice)}</td>
              ...
              <div className="space-y-1 border-t pt-1.5 text-[12px]">
                ...
                <div className="mt-1.5 flex justify-between border-t pt-1.5 text-base font-bold">
                  <span>Tổng tiền hàng:</span>
                  <span className="text-primary">{formatCurrency(calculateProductTotal(billOrder))}</span>
                </div>
                ...
                <div className="mt-1 flex justify-between rounded-[1rem] bg-rose-50 p-1.5 text-sm font-bold">
                  <span>Tiền COD:</span>
          ...
          <div className="invoice-print-actions flex justify-end gap-3 border-t pt-4">
